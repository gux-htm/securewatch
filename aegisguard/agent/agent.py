"""
AegisGuard – Client Agent Daemon
...
class AgentConfig:
    ...
    cert_path: str        = os.environ.get("AEGIS_CERT", "/etc/aegisguard-agent/client.crt")
    key_path: str         = os.environ.get("AEGIS_KEY",  "/etc/aegisguard-agent/client.key")
    ca_path: str          = os.environ.get("AEGIS_CA",   "/etc/aegisguard-agent/ca.crt")
...
async def run_agent(config: AgentConfig):
    logger.info(f"AegisGuard Agent starting (device_id={config.device_id}, "
    ...
    try:
        ...
        observer.stop()
        observer.join()
        logger.info("AegisGuard Agent stopped.")


def main():
    """CLI entry point for the agent daemon."""
    import configparser
    config_file = os.environ.get("AEGIS_AGENT_CONFIG", "/etc/aegisguard-agent/agent.conf")
