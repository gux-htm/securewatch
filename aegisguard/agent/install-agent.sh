#!/usr/bin/env bash
# =============================================================================
# AegisGuard – Client Agent Installation Script (Linux)
# Registers this endpoint with the central server and installs as systemd service.
# Run as root: sudo bash install-agent.sh
# =============================================================================
set -euo pipefail

AEGIS_SERVER="${AEGIS_SERVER_URL:-https://your-aegisguard-server:8443}"
AGENT_DIR="/opt/aegisguard-agent"
CONF_DIR="/etc/aegisguard-agent"
LOG_DIR="/var/log/aegisguard-agent"
...
read -rp "AegisGuard server URL [$AEGIS_SERVER]: " INPUT
...
info "Authenticating with AegisGuard server..."
...
openssl req -new -key "$CONF_DIR/client.key" \
    ...
    -subj "/C=US/O=AegisGuard/CN=$HOSTNAME"
chmod 400 "$CONF_DIR/client.key"

# ── Register device on server & get signed cert ───────────────────────────────
info "Registering device on AegisGuard server..."
...
cat > /etc/systemd/system/aegisguard-agent.service <<EOF
[Unit]
Description=AegisGuard Security Agent
...
systemctl enable --now aegisguard-agent
info "AegisGuard Agent installed and started ✓"
info "Check status: systemctl status aegisguard-agent"
info "View logs:    journalctl -u aegisguard-agent -f"
