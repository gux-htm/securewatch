...
# =============================================================================
# AegisGuard – Native Windows Installation Script
...
param(
    [string]$InstallPath = "C:\AegisGuard",
    [string]$DataPath    = "C:\AegisGuardData",
    [string]$LogPath     = "C:\AegisGuardLogs",
...
function Write-Step($msg)  { Write-Host "[AegisGuard] $msg" -ForegroundColor Green }
...
# ─── Create aegisguard Windows user account ───────────────────────────────────
function Create-ServiceUser {
    Write-Step "Creating AegisGuard service account..."
    $SvcPass = [System.Web.Security.Membership]::GeneratePassword(24, 4)
    $SecPass = ConvertTo-SecureString $SvcPass -AsPlainText -Force

    if (-not (Get-LocalUser "aegisguard" -ErrorAction SilentlyContinue)) {
        New-LocalUser "aegisguard" -Password $SecPass -Description "AegisGuard Security Service" -PasswordNeverExpires
        Add-LocalGroupMember -Group "Performance Monitor Users" -Member "aegisguard"
    }
    ...
    icacls "$InstallPath\svc_pass.enc" /inheritance:r /grant "aegisguard:(R)" | Out-Null
...
function Setup-PostgreSQL {
    ...
    & "$pgBin\psql.exe" -U postgres -c "CREATE USER aegisguard WITH PASSWORD '$PgPass';"   2>$null
    & "$pgBin\psql.exe" -U postgres -c "CREATE DATABASE aegisguard OWNER aegisguard;"      2>$null
    & "$pgBin\psql.exe" -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE aegisguard TO aegisguard;" 2>$null

    @"
DATABASE_URL=postgresql+asyncpg://aegisguard:${PgPass}@localhost:5432/aegisguard
PGHOST=localhost
PGPORT=5432
PGUSER=aegisguard
PGPASSWORD=${PgPass}
PGDATABASE=aegisguard
"@ | Set-Content "$InstallPath\db.env" -Encoding UTF8
    icacls "$InstallPath\db.env" /inheritance:r /grant "aegisguard:(R)" | Out-Null
...
function Generate-Certificates {
    ...
    & $opensslBin req -new -x509 -days 3650 -key "$ca\ca.key" `
        -out "$ca\ca.crt" -subj "/C=US/O=AegisGuard/CN=AegisGuard Root CA"
    & $opensslBin genrsa -out "$crt\server.key" 4096
    & $opensslBin req -new -key "$crt\server.key" `
        -out "$crt\server.csr" -subj "/C=US/O=AegisGuard/CN=aegisguard-server"
    ...
    icacls "$ca\ca.key"     /inheritance:r /grant "aegisguard:(R)" | Out-Null
    icacls "$ca\master.key" /inheritance:r /grant "aegisguard:(R)" | Out-Null
...
"@ | Set-Content "$InstallPath\aegisguard.env" -Encoding UTF8
    icacls "$InstallPath\aegisguard.env" /inheritance:r /grant "aegisguard:(R)" | Out-Null
...
function Install-WindowsServices {
    Write-Step "Installing AegisGuard as Windows Services..."
    $nssm = "C:\ProgramData\chocolatey\bin\nssm.exe"
    $py   = "$InstallPath\venv\Scripts\uvicorn.exe"

    # Main server
    & $nssm install AegisGuard $py
    & $nssm set AegisGuard AppParameters "server.main:app --host 0.0.0.0 --port 8443 --ssl-keyfile `"$DataPath\certs\server.key`" --ssl-certfile `"$DataPath\certs\server.crt`""
    & $nssm set AegisGuard AppDirectory  "$InstallPath\src"
    & $nssm set AegisGuard AppEnvironmentExtra `
        "DATABASE_URL=$(Get-Content "$InstallPath\db.env" | Select-String "DATABASE_URL")"
    & $nssm set AegisGuard AppStdout     "$LogPath\server.log"
    & $nssm set AegisGuard AppStderr     "$LogPath\server-error.log"
    & $nssm set AegisGuard Start         SERVICE_AUTO_START
    & $nssm set AegisGuard ObjectName    ".\aegisguard"

    # IDS/IPS engine (must run as SYSTEM for raw sockets)
    $idsScript = "$InstallPath\src\server\services\ids_engine.py"
    & $nssm install AegisGuardIDS "$InstallPath\venv\Scripts\python.exe"
    & $nssm set AegisGuardIDS AppParameters  "-m server.services.ids_engine"
    & $nssm set AegisGuardIDS AppDirectory   "$InstallPath\src"
    & $nssm set AegisGuardIDS AppStdout      "$LogPath\ids.log"
    & $nssm set AegisGuardIDS AppStderr      "$LogPath\ids-error.log"
    & $nssm set AegisGuardIDS Start          SERVICE_AUTO_START

    # Start services
    Start-Service AegisGuard
    Start-Sleep 3
    Start-Service AegisGuardIDS
...
function Configure-Firewall {
    ...
    New-NetFirewallRule -DisplayName "AegisGuard Dashboard (HTTPS)" -Direction Inbound `
        -Protocol TCP -LocalPort 8443 -Action Allow -Profile Any -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "AegisGuard OpenVPN" -Direction Inbound `
...
function Print-Summary {
    ...
    Write-Host "  AegisGuard $AegisVersion – Installation Complete" -ForegroundColor Cyan
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "    Get-Service AegisGuard"
    Write-Host "    Restart-Service AegisGuard"
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
}

# ─── Main Execution ───────────────────────────────────────────────────────────
Write-Step "Starting AegisGuard $AegisVersion installation..."
Install-Chocolatey
