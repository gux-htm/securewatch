"""
AegisGuard – OpenVPN Network Manager
...
def create_network_vpn(network_id: int, name: str, subnet: str, port: int, protocol: str) -> str:
    ...
    server_cn = f"aegisguard-vpn-{network_id}"
    ...
    server_conf = textwrap.dedent(f"""\
        # AegisGuard – Network: {name} (id={network_id})
        port {port}
        proto {protocol}
        dev tun{network_id}
        ...
        ifconfig-pool-persist /var/lib/aegisguard/ovpn/{network_id}/ipp.txt
        ...
        persist-key
        persist-tun
        status /var/log/aegisguard/openvpn-{network_id}-status.log
        log-append /var/log/aegisguard/openvpn-{network_id}.log
        verb 3
        # Verify client certificates against AegisGuard CA only
        verify-client-cert require
        # Script hook: called by OpenVPN to verify client MAC+IP via AegisGuard API
...
def _install_vpn_service(network_id: int, conf_path: Path) -> None:
    ...
    service_name = f"aegisguard-vpn@{network_id}"
    unit_content = textwrap.dedent(f"""\
        [Unit]
        Description=AegisGuard VPN Network {network_id}
        After=network.target aegisguard.service
...
def stop_network_vpn(network_id: int) -> None:
    ...
    service = f"aegisguard-vpn@{network_id}"
...
def generate_client_ovpn(
    ...
    ovpn = textwrap.dedent(f"""\
        # AegisGuard Generated Client Configuration
...
def get_vpn_status(network_id: int) -> dict:
    ...
    status_path = Path(f"/var/log/aegisguard/openvpn-{network_id}-status.log")
