...
function LoginGate({ onLogin }: { onLogin: () => void }) {
  const [mac, setMac] = useState(localStorage.getItem("omg_mac") ?? "");
  const [sig, setSig] = useState(localStorage.getItem("omg_signature") ?? "");
  ...
  async function verify(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/public/device/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mac, signature: sig }),
      });
      const body = await res.json();
      if (!res.ok) throw new Error(body.error ?? "Verification failed");
      localStorage.setItem("omg_token", body.token);
      localStorage.setItem("omg_mac", mac);
      localStorage.setItem("omg_signature", sig);
      onLogin();
    } catch (err) { setError((err as Error).message); }
    finally { setLoading(false); }
  }

  const cls = "w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500 font-mono";
        ...
        <form onSubmit={verify} className="space-y-3">
          <div className="space-y-1">
            ...
            <input required value={mac} onChange={(e) => setMac(e.target.value.toUpperCase())}
              placeholder="AA:BB:CC:DD:EE:FF" className={cls} />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Digital Signature</label>
            <input required value={sig} onChange={(e) => setSig(e.target.value)}
              placeholder="Paste your signature..." className={cls} />
            <p className="text-[10px] text-slate-500">From your registration approval screen</p>
          </div>
          {error && (
          ...
          <button type="submit" disabled={loading}
            ...
            <LogIn size={15} /> {loading ? "Verifying..." : "Access Portal"}
          </button>
        </form>
...
function FileViewerModal({ file, onClose, onEdit }: { file: FileEntry; onClose: () => void; onEdit: () => void }) {
  const { data: content, isLoading, error } = useQuery({
    ...
    queryFn: () => apiFetch<string>(`/files/content?path=${encodeURIComponent(file.file_path)}`),
  });
  return (
    <div className="fixed inset-0 z-40 bg-black/80 flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-4 bg-slate-950">
        ...
        {content !== undefined && (
          <pre className="text-slate-200 font-mono text-xs leading-relaxed whitespace-pre-wrap">{content as unknown as string}</pre>
        )}
      </div>
...
function FileEditorModal({ file, onClose, onSaved }: { file: FileEntry; onClose: () => void; onSaved: () => void }) {
  ...
  if (draft === null && original !== undefined) setDraft(original as unknown as string);
  const content = draft ?? (original as unknown as string) ?? "";
...
import os
dst = r"artifacts/oh-my-guard/src/pages/EndDevicePortal.tsx"
